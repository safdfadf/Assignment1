...
namespace UnityStandardAssets.CinematicEffects
{
    ...
    public class SMAA : IAntiAliasing
    {
        ...
        public void OnRenderImage(Camera camera, RenderTexture source, RenderTexture destination)
        {
            ...
            // Pass IDs
            var passEdgeDetection = (int)settings.edgeDetectionMethod;
            var passBlendWeights = 4;
            var passNeighborhoodBlending = 5;
            var passResolve = 6;
            ...
            material.SetVector("_Params1",
                new Vector4(preset.threshold, preset.depthThreshold, preset.maxSearchSteps,
                    preset.maxDiagonalSearchSteps));
            material.SetVector("_Params2", new Vector2(preset.cornerRounding, preset.localContrastAdaptationFactor));

            material.SetMatrix("_ReprojectionMatrix",
                m_PreviousViewProjectionMatrix * Matrix4x4.Inverse(viewProjectionMatrix));

            var subsampleIndex = m_FlipFlop < 0.0f ? 2.0f : 1.0f;
            ...
            else if (predication.enabled)
            {
                ...
                material.SetVector("_Params3",
                    new Vector3(predication.threshold, predication.scale, predication.strength));
            }
            ...
            if (m_Accumulation == null || m_Accumulation.width != width || m_Accumulation.height != height)
            {
                ...
                m_Accumulation =
                    RenderTexture.GetTemporary(width, height, 0, source.format, RenderTextureReadWrite.Linear);
                m_Accumulation.hideFlags = HideFlags.HideAndDontSave;

                isFirstFrame = true;
            }

            var rt1 = TempRT(width, height, source.format);
            ...
            {
                var rt2 = TempRT(width, height, source.format);
        ...
        public struct QualitySettings
        {
            ...
            [Range(0, 100)] [Tooltip("Specifies how much sharp corners will be rounded.")]
